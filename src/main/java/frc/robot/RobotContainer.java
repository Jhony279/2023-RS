...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(driverJoystick, XboxController.Button.kA.value)
      .onTrue(new InstantCommand(() -> drive.setGear(Value.kForward)));
    // High gear
    new JoystickButton(driverJoystick, XboxController.Button.kB.value)
      .onTrue(new InstantCommand(() -> drive.setGear(Value.kReverse)));

    // Transerval forward
    new JoystickButton(driverJoystick, 7)
      .onTrue(new InstantCommand(() -> hang.setTransveral(Value.kForward)));

    // Transversal reverse
    new JoystickButton(driverJoystick, 8)
      .onTrue(new InstantCommand(() -> hang.setTransveral(Value.kReverse)));

    // Intake
    final JoystickButton intake = new JoystickButton(driverJoystick, XboxController.Button.kRightBumper.value);
    intake.whileTrue(intakeCmdGroup);
    intake.onFalse(intakeRetractCmdGroup);

    // Outake
    final JoystickButton outake = new JoystickButton(driverJoystick, XboxController.Button.kLeftBumper.value);
    outake.whileTrue(outakeCmdGroup);
    outake.onFalse(intakeRetractCmdGroup);
    ...
    toggleShooter.onFalse(enableShooterCmd);
    ...
    */
    ...
    enableFeeder.whileTrue(enableFeederCmd);
  }
